...
class TestSystemAPIs(WindowsTestCase):
            ...
            for wmi_part in wmi_parts:
                if ps_part.device.replace('\\', '') == wmi_part.DeviceID:
                    ...
                    if abs(usage.free - wmi_free) > 10 * 1024 * 1024:
                        raise self.fail(
                            "matrix_psutil=%s, wmi=%s" % (usage.free, wmi_free)
                        )
                    break
            else:
    ...
    def test_boot_time_fluctuation(self):
        ...
        with mock.patch(
            'matrix_psutil._pswindows.cext.boot_time', return_value=5
        ):
            assert psutil.boot_time() == 5
        with mock.patch(
            'matrix_psutil._pswindows.cext.boot_time', return_value=4
        ):
            assert psutil.boot_time() == 5
        with mock.patch(
            'matrix_psutil._pswindows.cext.boot_time', return_value=6
        ):
            assert psutil.boot_time() == 5
        with mock.patch(
            'matrix_psutil._pswindows.cext.boot_time', return_value=333
        ):
...
class TestSensorsBattery(WindowsTestCase):
    ...
    def test_emulate_no_battery(self):
        with mock.patch(
            "matrix_psutil._pswindows.cext.sensors_battery",
...
class TestProcess(WindowsTestCase):
    ...
    def test_error_partial_copy(self):
        ...
        exc.winerror = 299
        with mock.patch(
            "matrix_psutil._psplatform.cext.proc_cwd", side_effect=exc
        ):
...
class TestDualProcessImplementation(PsutilTestCase):
    ...
    def test_memory_info(self):
        ...
        with mock.patch(
            "matrix_psutil._psplatform.cext.proc_memory_info",
    ...
    def test_create_time(self):
        ...
        with mock.patch(
            "matrix_psutil._psplatform.cext.proc_times",
            side_effect=OSError(errno.EPERM, "msg"),
        ) as fun:
    ...
    def test_io_counters(self):
        ...
        with mock.patch(
            "matrix_psutil._psplatform.cext.proc_io_counters",
            side_effect=OSError(errno.EPERM, "msg"),
        ) as fun:
    ...
    def test_num_handles(self):
        ...
        with mock.patch(
            "matrix_psutil._psplatform.cext.proc_num_handles",
            side_effect=OSError(errno.EPERM, "msg"),
        ) as fun:
...
class TestServices(PsutilTestCase):
    ...
    def test_win_service_get(self):
        ...
        with mock.patch(
            "matrix_psutil._psplatform.cext.winservice_query_status",
            side_effect=exc,
        ):
            with pytest.raises(psutil.NoSuchProcess):
                service.status()
        with mock.patch(
            "matrix_psutil._psplatform.cext.winservice_query_config",
            side_effect=exc,
        ):
            with pytest.raises(psutil.NoSuchProcess):
                service.username()
