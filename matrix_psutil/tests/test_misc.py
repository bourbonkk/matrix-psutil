...
class TestSpecialMethods(PsutilTestCase):
    ...
    def test_process__repr__(self, func=repr):
        ...
        r = func(p)
        assert "matrix_psutil.Process" in r
    ...
    def test_error__repr__(self):
        assert repr(psutil.Error()) == "matrix_psutil.Error()"
    ...
    def test_no_such_process__repr__(self):
        assert (
            ...
            == "matrix_psutil.NoSuchProcess(pid=321, msg='process no longer"
            " exists')"
        )
        assert (
    ...
    def test_zombie_process__repr__(self):
        ...
        assert (
            ...
            == "matrix_psutil.ZombieProcess(pid=321, ppid=320, name='name',"
            " msg='foo')"
        )
    ...
    def test_timeout_expired__repr__(self):
        assert (
            ...
            == "matrix_psutil.TimeoutExpired(seconds=5, msg='timeout after 5"
            " seconds')"
        )
        assert (
...
class TestMisc(PsutilTestCase):
    def test__all__(self):
        ...
        # Can't do `from matrix_psutil import *` as it won't work on python 3
...
class TestCommonModule(PsutilTestCase):
    ...
    def test_supports_ipv6(self):
        ...
        if supports_ipv6():
            ...
            supports_ipv6.cache_clear()
            with mock.patch(
                'matrix_psutil._common.socket.socket', side_effect=socket.error
            ) as s:
                assert not supports_ipv6()
                assert s.called
            ...
            supports_ipv6.cache_clear()
            with mock.patch(
                'matrix_psutil._common.socket.socket',
            ...
            supports_ipv6.cache_clear()
            with mock.patch(
                'matrix_psutil._common.socket.socket.bind',
                side_effect=socket.gaierror,
            ) as s:
                assert not supports_ipv6()
                supports_ipv6.cache_clear()
    ...
    def test_isfile_strict(self):
        ...
        with mock.patch(
            'matrix_psutil._common.stat.S_ISREG', return_value=False
        ):
            assert not isfile_strict(this_file)
