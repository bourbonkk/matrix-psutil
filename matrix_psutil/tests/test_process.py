...
from matrix_psutil.tests import create_c_exe
from matrix_psutil.tests import create_py_exe
...
class TestProcess(PsutilTestCase):
    ...
    def test_send_signal_mocked(self):
        ...
        p = self.spawn_psproc()
        with mock.patch(
            'matrix_psutil.os.kill', side_effect=OSError(errno.ESRCH, "")
        ):
        ...
        p = self.spawn_psproc()
        with mock.patch(
            'matrix_psutil.os.kill', side_effect=OSError(errno.EPERM, "")
        ):
    ...
    def test_oneshot_twice(self):
        ...
        with mock.patch("matrix_psutil._psplatform.Process.cpu_times") as m1:
            with mock.patch(
                "matrix_psutil._psplatform.Process.oneshot_enter"
            ) as m2:
                with p.oneshot():
                    p.cpu_times()
                    p.cpu_times()
    ...
    def test_zombie_process_is_running_w_exc(self):
        ...
        p = psutil.Process()
        with mock.patch(
            "matrix_psutil.Process", side_effect=psutil.ZombieProcess(0)
        ) as m:
            assert p.is_running()
            assert m.called
    ...
    def test_zombie_process_status_w_exc(self):
        ...
        p = psutil.Process()
        with mock.patch(
            "matrix_psutil._psplatform.Process.status",
