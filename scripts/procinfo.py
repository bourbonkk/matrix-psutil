...
def print_(a, b):
    if sys.stdout.isatty() and matrix_psutil.POSIX:
...
def run(pid, verbose=False):
    ...
    except matrix_psutil.NoSuchProcess as err:
        sys.exit(str(err))
    ...
    if hasattr(proc, "rlimit"):
        res_names = [x for x in dir(matrix_psutil) if x.startswith("RLIMIT")]
        resources = []
        for res_name in res_names:
            try:
                soft, hard = proc.rlimit(getattr(matrix_psutil, res_name))
            except matrix_psutil.AccessDenied:
                pass
            else:
        ...
        if resources:
            ...
            for res_name, soft, hard in resources:
                if soft == matrix_psutil.RLIM_INFINITY:
                    soft = "infinity"
                if hard == matrix_psutil.RLIM_INFINITY:
                    hard = "infinity"
                print_('', template % (
